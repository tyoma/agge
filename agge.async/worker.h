...
#include "../libraries/polyq/polyq/queue.h"

#include <agge/hybrid_event.h>
#include <agge/thread.h>

namespace agge {
template <typename WorkOutT> class worker : noncopyable {
public:
  struct work_in;
...
template <typename WorkOutT> struct worker<WorkOutT>::work_in {
  virtual ~work_in() {}
  virtual void run(typename worker<WorkOutT>::out_queue_type &queue) = 0;
};

template <typename WorkOutT> class worker<WorkOutT>::consumer {
public:
  consumer(typename worker<WorkOutT>::out_queue_type &queue) : _queue(queue) {}

  void operator()(work_in &w) const { w.run(_queue); }
...
inline worker<WorkOutT>::worker(in_queue_type &input, out_queue_type &output)
    : _input(input), _output(output),
      _worker_thread(&worker::worker_proc, this) {}
#pragma warning(default : 4355)

template <typename WorkOutT> inline worker<WorkOutT>::~worker() {
  _input.stop();
}
