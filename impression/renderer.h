...
namespace agg2
{
	...
	inline void renderer_base<PixelFormat>::clear(const color_type& c)
	{
		...
		if(width())
		{
			for(y = 0; y < height(); y++)
			{
				m_target.copy_hline(0, y, width(), c);
			}
		}
	}

	template <class PixelFormat>
	inline void renderer_base<PixelFormat>::blend_solid_hspan(int x, int y, int len, const color_type& c, const agg::cover_type* covers)
	{
		if (y < ymin() || ymax() < y)
			return;
		if (x < xmin())
	...
	template <class PixelFormat>
	inline void renderer_base<PixelFormat>::blend_hline(int x1, int y, int x2, const color_type& c, agg::cover_type cover)
	{
		if (y < ymin() || ymax() < y)
			return;
		if (x1 > x2)
	...
	inline void render_scanlines_aa_solid(Rasterizer& ras, Scanline& sl, BaseRenderer& ren, const ColorT& color)
	{
			...
			for (int y = ras.min_y(); y <= ras.max_y(); ++y)
			{
				...
				agg2::render_scanline_aa_solid(sl, y, ren, ren_color);
			}
	}
}
